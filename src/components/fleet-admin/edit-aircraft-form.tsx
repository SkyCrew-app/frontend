...
import { UPDATE_AIRCRAFT } from "@/graphql/planes"
import {
  type Aircraft,
  AvailabilityStatus,
...
export function EditAircraftForm({ aircraft, isOpen, onClose, onSuccess }: EditAircraftFormProps) {
  const [formData, setFormData] = useState<CreateAircraftInput>({
    ...
    availability_status: aircraft.availability_status as AvailabilityStatus,
    maintenance_status: aircraft.maintenance_status,
    hourly_cost: aircraft.hourly_cost,
    documents_url: aircraft.documents_url || [],
  })

  const [imageFile, setImageFile] = useState<File | null>(null)
  const [documentFile, setDocumentFile] = useState<File | null>(null)
  const [imagePreview, setImagePreview] = useState<string | null>(aircraft.image_url || null)
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        hourly_cost: aircraft.hourly_cost,
        documents_url: aircraft.documents_url || [],
        ...(aircraft.maxAltitude !== undefined && { maxAltitude: aircraft.maxAltitude }),
        ...(aircraft.cruiseSpeed !== undefined && { cruiseSpeed: aircraft.cruiseSpeed }),
  ...
  const [updateAircraft, { loading }] = useMutation<UpdateAircraftResponse>(UPDATE_AIRCRAFT, {
    onCompleted: () => {
      toast({
        title: "Aéronef mis à jour",
        description: "L'aéronef a été mis à jour avec succès.",
      })
      onSuccess()
    },
    onError: (error) => {
      toast({
        title: "Erreur",
        description: `Erreur lors de la mise à jour de l'aéronef: ${error.message}`,
        variant: "destructive",
      })
    },
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.maxAltitude !== undefined) (filteredFormData as any)["maxAltitude"] = formData.maxAltitude
    if (formData.cruiseSpeed !== undefined) (filteredFormData as any)["cruiseSpeed"] = formData.cruiseSpeed
    if (formData.consumption !== undefined) (filteredFormData as any)["consumption"] = formData.consumption
    if (formData.current_location !== undefined)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Modifier l'aéronef {aircraft.registration_number}</DialogTitle>
        </DialogHeader>
                        ...
                        <div className="flex flex-col items-center justify-center pb-6 pt-5">
                          ...
                          {aircraft.documents_url && aircraft.documents_url.length > 0 && (
                            <p className="mt-2 text-xs font-medium text-primary">
                              {aircraft.documents_url.length} document(s) existant(s)
                            </p>
                          )}
                        </div>
                        <input
