    ...
    <Card className={`border ${borderColor}`}>
      <CardHeader className={`${bgColor}`}>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>{type === "departure" ? t('departureAirport') : t('arrivalAirport')}</span>
      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex flex-col space-y-2">
          <h3 className="text-lg font-semibold">{airportInfo.name || t('aiport')}</h3>
        ...
        <Tabs defaultValue="general" className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            ...
            <TabsTrigger value="runways">{t('runway')}</TabsTrigger>
            <TabsTrigger value="frequencies">{t('frequency')}</TabsTrigger>
            ...
            <ScrollArea className="h-48 w-full rounded-md border p-4">
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <p>
                    <strong>{t('elevation')}:</strong> {airportInfo.elevation?.toFixed(2) || "N/A"} ft
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Sun className="h-5 w-5 text-blue-500" />
                  <p>
                    <strong>{t('timezone')}:</strong> {airportInfo.timezone?.name || "N/A"}
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Sunrise className="h-5 w-5 text-blue-500" />
                  <p>
                    <strong>{t('sunrise')}:</strong> {formatTime(airportInfo.times?.sunrise)}
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Sunset className="h-5 w-5 text-blue-500" />
                  <p>
                    <strong>{t('sunset')}:</strong> {formatTime(airportInfo.times?.sunset)}
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Compass className="h-5 w-5 text-blue-500" />
                  <p>
                    <strong>{t('coordinates')}:</strong> {airportInfo.lat?.toFixed(4) || "N/A"}° N,{" "}
          ...
          <TabsContent value="runways">
            <ScrollArea className="h-48 w-full rounded-md border p-4">
              {airportInfo.runways && airportInfo.runways.length > 0 ? (
                    ...
                    <li key={index} className="bg-muted p-3 rounded-md">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div className="flex items-center">
                          ...
                          <span>{t('length')}: {(runway.length / 1000).toFixed(2)} km</span>
                        </div>
                        <div className="flex items-center">
                          <Ruler className="h-4 w-4 mr-1 text-muted-foreground rotate-90" />
                          <span>{t('width')}: {runway.width?.toFixed(0) || "N/A"} m</span>
                        </div>
                      </div>
                    </li>
                  ))}
                ...
                <p className="text-center text-muted-foreground py-4">{t('noInformationRunway')}</p>
              )}
            </ScrollArea>
          </TabsContent>
          <TabsContent value="frequencies">
            <ScrollArea className="h-48 w-full rounded-md border p-4">
              {airportInfo.frequencies && airportInfo.frequencies.length > 0 ? (
                ...
                <p className="text-center text-muted-foreground py-4">{t('noInformationFrequency')}</p>
              )}
            </ScrollArea>
          </TabsContent>
            ...
            <ScrollArea className="h-48 w-full rounded-md border p-4">
              <div className="space-y-4">
                {weather ? (
                  <div className="bg-muted p-4 rounded-md">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <p>
                          <strong>{t('temperature')}:</strong> {weather.temperature.toFixed(1)}°C
                        </p>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Cloud className="h-4 w-4 text-blue-500" />
                        <p>
                          <strong>{t('conditions')}:</strong> {weather.conditions}
                        </p>
                      </div>
                      {weather.wind && (
                        <>
                          <div className="flex items-center space-x-2">
                            ...
                            <p>
                              <strong>{t('wind')}:</strong> {weather.wind.speed} kt
                            </p>
                          </div>
                          <div className="flex items-center space-x-2">
                            <Compass className="h-4 w-4 text-blue-500" />
                            <p>
                              <strong>{t('direction')}:</strong> {weather.wind.direction}°
                            </p>
                          </div>
                        </>
                      )}
                  ...
                  <p className="text-center text-muted-foreground py-4">{t('noMeteoInformation')}</p>
                )}

                <div>
                  <p className="font-semibold mb-2">{t('metar')}:</p>
                  <p className="text-sm font-mono bg-muted p-2 rounded overflow-x-auto">
                    {airportInfo.weather?.METAR || t('noDisplay')}
                  </p>
                </div>

                <div>
                  <p className="font-semibold mb-2">{t('taf')}:</p>
                  <p className="text-sm font-mono bg-muted p-2 rounded overflow-x-auto">
                    {airportInfo.weather?.TAF || t('noDisplay')}
                  </p>
                </div>
              </div>
            </ScrollArea>
